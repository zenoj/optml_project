...
def init_weights(world_size, rank, model, train_loader, adjacency_matrix, criterion):
...
def motef_worker(rank, world_size, model, train_loader, val_loader, epochs, gamma, eta, lambda_, comp_func, com_ratio, adjacency_matrix):
    device = torch.device('cpu')
    model = model.to(device)
    ...
    g, v, m, x, h, q_h_i, q_g_i, neighborStates, weights = init_weights(world_size, rank, model, train_loader, adjacency_matrix, criterion)
...
def worker_fn(rank, world_size, model, trainset, valset, epochs, gamma, eta, lambda_, comp_func, com_ratio, adjacency_matrix):
    ...
    motef_worker(rank, world_size, model, train_loader, val_loader, epochs, gamma, eta, lambda_, comp_func, com_ratio, adjacency_matrix)
...
def run_motef(world_size, epochs, gamma, eta, lambda_, comp_func, com_ratio, topology, prob=0.1):
    ...
    mp.spawn(worker_fn, args=(world_size, model, train_set, val_set, epochs, gamma, eta, lambda_, comp_func, com_ratio, adjacency_matrix), nprocs=world_size)
...
if __name__ == "__main__":
    ...
    for gam, et, lbd, com, topology in itertools.product(gammas, etas, lbds, coms, topologies):
        ...
        run_motef(world_size=world_size, epochs=ep, gamma=gam, eta=et, lambda_=lbd, comp_func=comp_func, com_ratio=com, topology=topology, prob=prob)
